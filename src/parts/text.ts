...
export class Text extends MyDisplay {
  ...
  private _updateItemSize(): void {
    ...
    const fontSize = Math.min(sw, sh) * Func.val(0.5, 0.2);
    ...
    const r = ~~(Util.map(this._fixRate.val, 0, this._items.length - 1, 0, 0.75));
  ...
  protected _update(): void {
    ...
    const mx = MousePointer.instance.easeNormal.y;
    this._items.forEach((val,i) => {
      const fixKey = i;
      const ix = ~~(fixKey / this._line);
      const iy = ~~(fixKey % this._line);

      if(this._changeCnt % 2 == 0) {
        Tween.set(val.el, {
          x: (mx * Math.max(Func.sh(), Func.sw()) * 0.2) * (iy >= this._line * 0.5 ? 1 : -1),
          y: 0,
        })
      } else {
        Tween.set(val.el, {
          x: 0,
          y: (mx * Math.max(Func.sh(), Func.sw()) * 0.2) * (ix >= this._line * 0.5 ? 1 : -1),
        })
      }
    })
